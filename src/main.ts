...
async function run() {
  try {
    ...
    if (tag) {
      ...
      switch (action) {
        ...
        default:
          ...
          core.warning(
            `Cannot upload assets for release which is being ${action}\n\nPayload: ${JSON.stringify(
    ...
    const {
      data: { upload_url: url },
    } = await octokit.repos.getRelease({ ...repo, release_id });
    const { data: existingAssets } = await octokit.repos.listAssetsForRelease({
      ...repo,
      release_id,
    });
